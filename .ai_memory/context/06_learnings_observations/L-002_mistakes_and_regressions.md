...
- **[2026-07-27] ROUTING RESTRUCTURE:** Renamed `index.html` (SPA) → `app.html` and `landingpage.html` → `index.html` so Vercel serves the landing page at `/`. Updated all 3 CTA links in the new `index.html` to point to `app.html`. Updated T-001 architecture doc with routing table.
- **[2026-07-27] FEATURE: Supabase Auth (Google OAuth + Email/Password).** Added auth guard in `init()`, Auth Modal overlay, Google OAuth sign-in, email/password sign-in/sign-up, profile dropdown with avatar/name/email from `user_metadata`, Sign Out handler, Profile Settings modal, and sender data association from authenticated user. Updated T-001.
